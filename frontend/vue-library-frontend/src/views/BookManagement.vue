...
// 新增：图书数量统计，用于页眉显示
const totalBooks = computed(() => tableData.value.length)

// 新增：简单的数字转中文（仅在 2 时返回“两”，其他返回数字字符串）
const numberToChinese = (n) => {
  const val = typeof n === 'number' ? n : (n?.value ?? 0)
  if (val === 2) return '两'
  return String(val)
}
...
<template>
  <div class="page-card">
    <div class="page-header">
      ...
      <div class="stats">共{{ numberToChinese(totalBooks) }}种</div>
    </div>
    ...
    <div class="actions-bar">
      <div class="left-actions">
        <el-button type="primary" @click="handleAdd"
          ><i class="el-icon-plus"></i> 新增图书</el-button
        >
      </div>
...
.page-card {
  ...
  border-radius: 12px;
  padding: 20px;
  box-shadow: 0 6px 18px rgba(35, 47, 68, 0.08);
}
.page-header {
  display: flex;
...
.action-buttons .el-button--info {
  /* 提供最低限度的样式以避免空的 ruleset（可根据主题进一步微调） */
  box-shadow: none;
